...
export async function pruneCommand(options) {
    ...
    if (isPreview) {
        ...
        if (unusedImportsCount > 0)
            console.log(chalk.green(`  ✓ Unused imports to remove: ${unusedImportsCount}`));
        if (unusedVariablesCount > 0)
            console.log(chalk.green(`  ✓ Unused variables to remove: ${unusedVariablesCount}`));
        if (unusedDeclarationsCount > 0)
            console.log(chalk.green(`  ✓ Unused declarations to remove: ${unusedDeclarationsCount}`));
        if (consoleRemovedCount > 0)
            console.log(chalk.green(`  ✓ Console logs to remove: ${consoleRemovedCount}`));
        if (commentsRemovedCount > 0)
            console.log(chalk.green(`  ✓ Comments to remove: ${commentsRemovedCount}`));
        if (unusedFiles.length > 0)
            console.log(chalk.green(`  ✓ Files to delete: ${unusedFiles.length}`));
        if (unusedDeps.length > 0)
            console.log(chalk.green(`  ✓ Unused dependencies: ${unusedDeps.length}`));
        ...
        if (selectedFeatures.includes("removeEmpty")) {
            ...
            if (emptyCount > 0)
                console.log(chalk.green(`  ✓ Empty files to remove: ${emptyCount}`));
        }
        ...
        if (unusedDeclarationsCount > 0)
            console.log(chalk.green(`  ✓ Removed unused declarations: ${unusedDeclarationsCount}`));
        if (consoleRemovedCount > 0)
